...
import { ipRangeCheck } from "../index";

describe("#ipRangeCheck", function () {
  ...
  describe("for Ipv4", function () {
    ...
    it("should succeed when the IP is in the range", function () {
      ...
      assert.equal(
        true,
        ipRangeCheck("192.168.1.1", ["102.1.5.0/24", "192.168.1.0/24"])
      );
      for (let i = 0; i <= 255; i++) {
    ...
    describe("transmitted over IPv6", function () {
      it("should match IPv4 CIDR", function () {
        assert.equal(
          true,
          ipRangeCheck("0:0:0:0:0:FFFF:222.1.41.90", "222.1.41.90")
        );
        assert.equal(
          true,
